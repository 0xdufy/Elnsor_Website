...
try {
  for (const path of ['/products.html', '/en/products.html']) {
    ...
    const disabledFixtureCount = await page.locator('[data-product-id="disabled-test-product"]').count();
    ...
    expect(new Set(result.categoryIds).size === result.categoryIds.length, 'category IDs are unique', {
      path,
      result,
    });
    ...
    expect(result.favaImageCount === 0, 'missing image renders without a substitute', { path, result });
    ...
    const rawFilter = await page.locator('[data-products-grid] .product-card:not([hidden])').count();
    await page.click('[data-filter="finished"]');
    const finishedFilter = await page.locator('[data-products-grid] .product-card:not([hidden])').count();
    ...
    const quote = await page.locator('[data-product-id="soybean-hulls-fine"] .text-link').getAttribute('href');
  ...
  for (const path of ['/index.html', '/en/index.html']) {
    ...
    const homepage = await page.evaluate(() => ({
      individualSoybeanCards: document.querySelectorAll('[data-product-id^="soybean-hulls-"]').length,
      soybeanFamily: document.querySelector('[data-family-id="soybean-hulls"]')?.dataset.productIds,
      finishedFamily: document.querySelector('[data-family-id="finished-feed"]')?.dataset.productIds,
    ...
    expect(Boolean(homepage.finishedFamily), 'homepage supports grouped finished feed', { path, homepage });
    expect(Boolean(homepage.quote?.includes('https://wa.me/201022232052?text=')), 'family quote is encoded', {
      path,
      homepage,
    });
    await page.close();
